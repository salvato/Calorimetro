...
CMsgWindow::CMsgWindow(CString Title/*="Messges"*/, CWnd* pParent) {
  pMessages = NULL;
  ...
  BOOL result;
  if(pParent == NULL) 
    result = CreateEx(exStyle, ClassName, Title, Style, 0, 40, 656, 480, 0, 0);
  else
    result = CreateEx(exStyle, ClassName, Title, Style, 0, 40, 656, 480, pParent->m_hWnd, NULL);
  pmyFont = NULL;
}
...
CMsgWindow::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if(strValue != "") {
    int curPos = 0;
    x0   = _ttoi(strValue.Tokenize(_T(","), curPos));
    y0   = _ttoi(strValue.Tokenize(_T(","), curPos));
    wdth = _ttoi(strValue.Tokenize(_T(","), curPos));
    hght = _ttoi(strValue.Tokenize(_T("\0"), curPos));
  }
  CRect Rect;
  GetWindowRect(&Rect);
